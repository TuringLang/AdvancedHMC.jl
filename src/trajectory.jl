...
function sample_phasepoint(rng, integrator, tc, ::Type{MetropolisTS}, h, z)
...
function sample_phasepoint(rng, integrator, tc, ::Type{MultinomialTS}, h, z)
    n_steps = abs(tc.L)
...
"""
function Termination(s::SliceTS, tc, H0::F, H′::F) where {F<:AbstractFloat}
    return Termination(false, !(s.ℓu < tc.Δ_max + -H′))
end
...
Check termination of a Hamiltonian trajectory.
"""
function Termination(s::MultinomialTS, tc, H0::F, H′::F) where {F<:AbstractFloat}
    return Termination(false, !(-H0 < tc.Δ_max + -H′))
end
