            ...
            @testset "$τsym" for (τsym, κ) in Dict(
                ...
                @testset  "NoAdaptation" begin
                    ...
                    if "GEWEKE_TEST" in keys(ENV) && ENV["GEWEKE_TEST"] == "1"
                        res = perform(GewekeTest(5_000), mvntest, x -> rand_θ_given(x, mvntest, metric, κ), g; progress=false)
                        p = plot(res, mvntest)
                        display(p)
                        println()
                    end
                ...
                @testset "$adaptorsym" for (adaptorsym, adaptor) in Dict(
                    ...
                    κ_used = if adaptorsym == :MassMatrixAdaptorOnly
                        κ
                    else
                        ϵ = find_good_stepsize(h, θ_init)
                        τ = reconstruct(κ.τ, integrator=reconstruct(κ.τ.integrator, ϵ=ϵ))
                        reconstruct(κ, τ=τ)
                    end
