    ...
    @testset "$metricsym" for (metricsym, metric) in Dict(
        ...
        @testset "$lfsym" for (lfsym, lf) in Dict(
            ...
            @testset "$τsym" for (τsym, τ) in Dict(
                :(StaticTrajectory{EndPointTS}) =>
                    StaticTrajectory{EndPointTS}(lf, n_steps),
                :(StaticTrajectory{MultinomialTS}) =>
                    StaticTrajectory{MultinomialTS}(lf, n_steps),
                ...
                :(NUTS{MultinomialTS,Generalised}) =>
                    NUTS{MultinomialTS,GeneralisedNoUTurn}(lf),
            )
                ...
                @testset "$adaptorsym" for (adaptorsym, adaptor) in Dict(
                    ...
                    τ_used = adaptorsym == :MassMatrixAdaptorOnly ? τ :
                        reconstruct(
                        τ,
                        integrator = reconstruct(lf, ϵ = find_good_stepsize(h, θ_init)),
                    )
                    samples, stats = sample(
                        h,
                        τ_used,
...
@testset "drop_warmup" begin
    ...
    h = Hamiltonian(metric, ℓπ, ∂ℓπ∂θ)
    τ = NUTS(Leapfrog(ϵ))
    adaptor = StanHMCAdaptor(MassMatrixAdaptor(metric), StepSizeAdaptor(0.8, τ.integrator))
    samples, stats = sample(
        h,
        τ,
        θ_init,
